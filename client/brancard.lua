...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
			...
			exports['Nico_Notify']:Alert('Info', 'Etwas ist schief gelaufen, mein Freund...', 10000, 'error', true)
        end
    else
        --QBCore.Functions.Notify("Your not near a ambulance..", 'error')
        --exports['mythic_notify']:SendAlert('error', 'Your not near a ambulance..')
		exports['Nico_Notify']:Alert('Info', 'Du bist nicht in der Nähe einer Ambulanz...', 10000, 'error', true)
    end
end)
...
AddEventHandler('hospital:client:RemoveBrancard', function()
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
			...
			exports['Nico_Notify']:Alert('Info', 'Du bist zu weit weg!', 10000, 'error', true)
        end
    end
end)
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
			...
			exports['Nico_Notify']:Alert('Info', 'Die Trage ist bereits im Einsatz.', 10000, 'error', true)
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
			...
			exports['Nico_Notify']:Alert('Info', 'Diese Branchenkarte ist bereits in Gebrauch!', 10000, 'error', true)
            IsAttached = false
        end
    end
end)
